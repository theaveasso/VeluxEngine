...
LIB_PATH :: (
	...
	else "lib/box3d.lib"              when ODIN_OS == .Windows && !BOX3D_SHARED
	else "lib/shared/box3d.lib"       when ODIN_OS == .Windows && BOX3D_SHARED
	else ""
)

when LIB_PATH != "" {
	when !#exists(LIB_PATH) {
		#panic("Could not find the compiled Box3D library at \"" + LIB_PATH + "\", it is built by velux's setup.bat")
	}
