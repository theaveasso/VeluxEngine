...
create_render_target :: proc(
	...
	create_info := render_target_create_info(width, height, color_format, depth_format)

	extent := vk.Extent3D{create_info.width, create_info.height, 1}
	rt.image = create_image(image_create_info(create_info.color_format, extent, {.SAMPLED, .COLOR_ATTACHMENT}), loc)
	rt.depth = create_image(image_create_info(create_info.depth_format, extent, {.DEPTH_STENCIL_ATTACHMENT}), loc)
...
destroy_render_target :: proc(target: ^Render_Target, loc := #caller_location) {
	wait_for_idle(loc)
	destroy_gpu_image(&target.image, loc)
	destroy_gpu_image(&target.depth, loc)
	target^ = {}
}
