...
begin_frame :: proc(device: ^Device) -> (frame: Frame, err: Error) {
	...
	vk_check(
		vk.WaitForFences(device.device, 1, &frame_data.in_flight_fence, true, max(u64)),
		.Vulkan_Call_Failed,
	) or_return
	...
	vk_check(
		vk.ResetFences(device.device, 1, &frame_data.in_flight_fence),
		.Vulkan_Call_Failed,
	) or_return

	vk_check(
		vk.ResetCommandBuffer(frame_data.command_buffer, {.RELEASE_RESOURCES}),
		.Vulkan_Call_Failed,
	) or_return

	begin_info: vk.CommandBufferBeginInfo = init_command_buffer_begin_info({.ONE_TIME_SUBMIT})
	vk_check(
		vk.BeginCommandBuffer(frame_data.command_buffer, &begin_info),
		.Vulkan_Call_Failed,
	) or_return
...
end_frame :: proc(device: ^Device, frame: Frame) -> (err: Error) {
	...
	vk_check(
		vk.QueueSubmit2(device.graphics_queue, 1, &submit_info, frame.in_flight_fence),
		.Vulkan_Call_Failed,
	) or_return

	present_info := init_present_info(
